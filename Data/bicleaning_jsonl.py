...
# I uploaded the train, val and test sets (in the json-lines format) into the Colab in the left side bar
input_jsonl = "/content/train.jsonl"
tsv_path = "/content/train.en-de"
...
with open(input_jsonl, "r", encoding="utf-8") as infile, open(tsv_path, "w", encoding="utf-8") as outfile:
    for line in infile:
      ...
      continue
      en = obj["translation"].get("en", "").strip().replace("\t", " ")
      de = obj["translation"].get("de", "").strip().replace("\t", " ")
      if en and de:
...
with open(input_jsonl, "r", encoding="utf-8") as infile, open(tsv_path, "w", encoding="utf-8") as outfile:
    for i, line in enumerate(infile):
        ...
        en = escape_special_chars(obj["translation"].get("en", "").strip())
        de = escape_special_chars(obj["translation"].get("de", "").strip())
...
with open("/content/train.clean", "r", encoding="utf-8") as infile, open(clean_jsonl, "w", encoding="utf-8") as outfile:
    for line in infile:
...
with open(clean_jsonl, "r", encoding="utf-8") as file:
